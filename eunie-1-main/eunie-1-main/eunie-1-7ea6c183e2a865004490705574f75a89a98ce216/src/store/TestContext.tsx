...
export const TestProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const retryAIAnalysis = useCallback(async () => {
    ...
    // Set loading state by clearing AI fields
    setReport(prev => prev ? { 
      ...prev, 
      isAiComplete: false, 
      todayTheme: undefined,
      cardInterpretation: undefined,
      psychologicalInsight: undefined,
      fiveElementAnalysis: undefined,
      reflection: undefined,
      actionSuggestion: undefined,
      pairInterpretations: undefined,
      isFallback: false 
    } : null);

    try {
      const aiAnalysis = await generateAIAnalysis(selectedCards, report.totalScores, language);
      const finalReport: AnalysisReport = {
        ...report,
        ...aiAnalysis,
        isAiComplete: true
      ...
      console.error("Retry AI Analysis failed:", err);
      // If it fails again, it will set isFallback: true via generateAIAnalysis
    }
  }, [report, selectedCards, language]);
