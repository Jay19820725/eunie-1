...
export const generateAIAnalysis = async (
  ...
  const langInstruction = currentLang === 'ja' 
    ? "【重要】必ず日本語 (ja-JP) で回答してください。中国語を混ぜないでください。また、各カードペアの解読 (pairInterpretations) を必ず含めてください。" 
    : "【重要】請務必使用繁體中文 (zh-TW) 回答。不要夾雜日文。同時，請務必包含每一組卡牌配對的解讀 (pairInterpretations)。";
  ...
  try {
    ...
    return {
      ...
      pairInterpretations: [
        { pair_id: "1", text: currentLang === 'ja' ? "このカードの組み合わせは、あなたの内なる静かな願いを映し出しています。" : "這組卡牌的碰撞，正輕聲訴說著妳內心深處的一段共鳴。" },
        { pair_id: "2", text: currentLang === 'ja' ? "今のあなたに必要なのは、自分自身を優しく受け入れることです。" : "現在的妳，最需要的是給予自己一份溫柔的接納。" },
        { pair_id: "3", text: currentLang === 'ja' ? "未来への一歩は、この小さな気づきから始まります。" : "通往未來的一步，正從這份小小的覺察中悄然開啟。" }
      ]
    };
  }
};
