    ...
    <div ref={reportRef} className="ma-container pt-12 md:pt-20 pb-48 md:pb-64 min-h-screen px-4 bg-[#FDFCF8]">
      ...
      {report.isFallback && (
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mb-12 p-6 rounded-2xl bg-fire/5 border border-fire/10 text-center"
        >
          <p className="text-xs text-fire/60 tracking-widest uppercase mb-2">{t('report_fallback_notice_title')}</p>
          <p className="text-sm text-ink-muted leading-relaxed max-w-lg mx-auto">
            {t('report_fallback_notice_desc')}
          </p>
          <button 
            onClick={retryAIAnalysis}
            className="mt-4 text-[10px] uppercase tracking-[0.4em] text-fire font-semibold hover:opacity-70 transition-opacity"
          >
            {t('report_retry_ai')}
          </button>
        </motion.div>
          ...
          <div className="flex flex-col gap-4 w-full max-w-xs">
            ...
            {report.isFallback && (
              <Button
                onClick={retryAIAnalysis}
                variant="outline"
                className="h-16 w-full gap-4 text-xs uppercase tracking-[0.4em] font-light border-fire/20 text-fire hover:bg-fire/5"
              >
                <Sparkles size={16} /> {t('report_retry_ai')}
              </Button>
            )}
            <Button
